...
export default function TimelineForm({ id, initial }: Props) {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      const url = isEdit ? `/api/timeline-items/${id}` : "/api/timeline-items";
  ...
  async function onDelete() {
    ...
    try {
      const res = await fetch(`/api/timeline-items/${id}`, { method: "DELETE" });
      const data = await res.json().catch(() => null);
      if (!res.ok || !data?.ok) {
