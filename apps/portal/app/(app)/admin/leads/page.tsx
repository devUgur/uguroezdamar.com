...
import { getLeadsPageData } from "@/src/server/leads";
...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  const { items, nextCursor } = await getLeadsPageData({ limit, cursor, q });
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link href={`/api/leads/exports?limit=1000${q ? `&q=${encodeURIComponent(String(q))}` : ""}`} className="px-4 py-2 bg-foreground text-background rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
            Export CSV
          </Link>
        </div>
      </div>
      ...
      <div className="flex justify-center">
        {nextCursor ? (
          <Link 
            href={`/leads?cursor=${nextCursor}&limit=${limit}${q ? `&q=${encodeURIComponent(String(q))}` : ""}`} 
