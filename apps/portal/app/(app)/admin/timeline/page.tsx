...
import { getTimelineItemsSnapshot } from "@/src/server/timeline";
...
export default async function TimelinePage() {
  const items = await getTimelineItemsSnapshot();
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/timeline/new" className="px-4 py-2 bg-foreground text-background rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
          New Entry
        </Link>
      </div>
              ...
              items.map((item) => (
                <tr key={item.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-4 text-right">
                    <Link href={`/admin/timeline/${item.id}`} className="text-xs text-blue-500 hover:underline mr-4">
                      Edit
                    </Link>
                  </td>
                </tr>
