...
import { getWorkItemsSnapshot } from "@/src/server/work";

export default async function WorkItemsPage() {
  const items = await getWorkItemsSnapshot();
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/work-items/new" className="px-4 py-2 bg-foreground text-background rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
          New Work Item
        </Link>
      </div>
              ...
              items.map((w) => (
                <tr key={w.slug} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-4 text-right">
                    <Link href={`/admin/work-items/${w.slug}`} className="text-xs text-blue-500 hover:underline mr-4">
                      Edit
                    </Link>
