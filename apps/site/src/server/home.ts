import { getProfile, getTimelineItems } from "@ugur/server";
...
function toSortTime(startIso: string, endIso?: string | null, isCurrent?: boolean): number {
...
export async function getTimelineForAbout(): Promise<AboutTimelineEntry[]> {
	try {
		const items = await getTimelineItems({ status: "published", limit: 100 });
		...
		const mapped = mixed
			.map((item) => ({
				...
				title: item.title,
				desc: item.description?.trim() || [item.organization, item.location].filter(Boolean).join(" • "),
			}))
