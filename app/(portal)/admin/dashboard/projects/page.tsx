import { getProjects } from "@/features/projects/server/mongo";
import Link from "next/link";

export default async function AdminProjectsPage() {
  const projects = await getProjects({ includeDrafts: true, limit: 500 });
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/dashboard/projects/new" className="px-4 py-2 bg-foreground text-background rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
          New Project
        </Link>
      </div>
              ...
              projects.map((project) => (
                <tr key={project.slug} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-4 text-right">
                    ...
                    <Link href={`/admin/dashboard/projects/${project.slug}`} className="text-xs text-blue-500 hover:underline">
                      Edit
                    </Link>
                  </td>
                </tr>
