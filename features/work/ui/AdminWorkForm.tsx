...
export default function AdminWorkForm({ id, initial }: { id?: string; initial?: WorkInitial }) {
  ...
  async function saveItem(event: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const url = isEdit ? `/api/admin/work-items/${id}` : "/api/admin/work-items";
      ...
      if (!isEdit && saved?.slug) {
        router.push(`/admin/dashboard/work-items/${saved.slug}`);
      }
      router.refresh();
    } catch (err: any) {
