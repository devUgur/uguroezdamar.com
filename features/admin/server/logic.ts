...
export async function createAdminInvite(email: string, invitedByEmail: string, role: "owner" | "admin" | "editor" = "editor") {
  ...
  // In development, return raw token so devs can test easily
...
export async function acceptAdminInvite(rawToken: string, password: string) {
  ...
  const res = await adminsCol.insertOne(doc as any);
...
export async function requireAdmin(request: NextRequest): Promise<RequireAdminResult> {
  ...
  const s = await getAdminSession(sessionId);
  if (!s) return { ok: false, reason: "invalid_session" };
  // lookup admin record for role checks
  const admin = await getAdminByEmail(s.email);
  return { ok: true, method: "session", session: s, admin: admin ?? null };
}
